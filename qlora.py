...
class SavePeftModelCallback(transformers.TrainerCallback):
    def save_model(self, args, state, kwargs):
        ...
        saved_file = s3.upload_file(peft_model_path, s3_bucket, s3_model_key)
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    if not args.full_finetune:
        if checkpoint_dir is not None:
            ...
            model = PeftModel.from_pretrained(model, join(
                checkpoint_dir, 'adapter_model'), is_trainable=True)
        else:
            ...
            def on_evaluate(self, args, state, control, model, **kwargs):
                ...
                for batch in tqdm(data_loader, total=len(data_loader)):
                    ...
                    for i, logit in enumerate(logits):
                        label_non_zero_id = (
                            batch['labels'][i] != -100).nonzero()[0][0]
                ...
                results[f'mmlu_{args.mmlu_split}_accuracy'] = np.mean(
                    subject_scores)
                trainer.log(results)
